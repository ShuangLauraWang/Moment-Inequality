function domegdt=M80_domegdtM3(XMat,dM,sH,Kx,dxidt,dS,mkup)
% XMat=[ones(nobs,1),fare, noconn,hub, time, dept, lcc,tour];
...
for i=1:dM.nC
    ...
    for j=1:ntype
        ...
        kx2=Kx(id1,3*j-2);  %fare
        kx3=Kx(id1,3*j-1);  %nconn
        kx1=Kx(id1,3*j);    %ndest
        kx4=Kx(id1,3*ntype+j);
        
        tx2=XMat(id1:id2,1);    %fare
        tx3=XMat(id1:id2,2);    %nconn
        tx1=XMat(id1:id2,3);    %ndest
        ...
        for k = 1: ((ncoef+1)*ntype)
            ...
            if k==3*j-2     %partial q/partial alpha
