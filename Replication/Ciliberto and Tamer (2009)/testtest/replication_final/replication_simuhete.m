...
%--------------------------------------------------------------------------%
                                                                           %
function sand = replication_simuhete(param)                                %
                                                                           %
global total r k  prob X rowX otherentry                                   %
...
%paramX=param(2:9);                                                        % Exogenous Variables
                                                                           %
paraheterog1 = param(2);                                                  % Market Presence
                                                                           %
paraheterog2 = param(3);                                                  % Opportunity Cost
                                                                           %
parafirmed = param(4:5);                                               % Competitive Effects

parafirmex  = param(6:9);
                                                                           %
%--------------------------------------------------------------------------%
...
roleheter = X(:,1:2)*paraheterog1;
roleheter=roleheter+X(:,3:4)*paraheterog2;

%--------------------------------------------------------------------------%
% (4) COMPETITIVE EFFECTS                                                  %
%--------------------------------------------------------------------------%
                                                                           %
onothereffected=zeros(total*size(X,1),k);                                    % The sum of the competitive effects change by market structure.
                                                                           %     Dimension: (#markets*(2^#firms)) X #firms
                                                                           %                                          
for i=1:k                                                                  %
    effect=repindex(:,i)*parafirmed(i)*ones(1,k);                           %
    effect(:,i)=zeros(total*size(X,1),1);                                  % Makes sure that a firm does not negatively affect its own profit.
    onothereffected=onothereffected+effect;                                    % Sum over all the firms in the market.
end
                                                                           %
clear effect                                                               %


onothereffectex=otherentry*parafirmex;
onothereffectex=onothereffectex*ones(1,k);
                                                                           %
%--------------------------------------------------------------------------%
% (5) CONSTANT TERM                                                        %
%--------------------------------------------------------------------------%
...
%--------------------------------------------------------------------------%
                                                                           %
common=roleheter(indexheter)+owneffect(diffdummies)...
    +onothereffectex(indexheter)+onothereffected;
                                                                           %
%--------------------------------------------------------------------------%
...
%--------------------------------------------------------------------------%
                                                                           %
[meanupp,meanlow,temp] = replication_simuloop(common);                                 %
                                                                           %
%--------------------------------------------------------------------------%
